...
function Form({refresh,data,setOpen}){
    ...
    const {
        ...
        isSuccess,
        mutate,
      } = useMutation(async (params) => await api_nestjs.post("/categories", params),
          {  
            onSuccess : (data) => {
    ...
    function handleSubmit(event){
        ...
        let {Category_name, category_image, Description } = event.target.elements

        let body = {
          strCategory: Category_name.value,
          strCategoryThumb: category_image.value,
          strCategoryDescription: Description.value
        };
            ...
            <div>
              <div>
                <p className="mt-1 text-sm text-gray-500">
                 Fill this form to add a Category
                </p>
              </div>
  ...
  <div className="mt-1">
    <textarea
      ...
      className="p-4 border-2 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full text-sm border-gray-300 rounded-md"
    />
  </div>
</div>
